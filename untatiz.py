...
def get_date():
    ...
    if now.hour < 14:
        # If before 2 PM, get the previous day's date
        date_to_display = now - timedelta(days=1)
    else:
        # If 2 PM or later, get the current day's date
        date_to_display = now
    ...
    for index in transaction.index:
        ...
        if new != "퐈":
            if (player_id.loc[new] == id).any():
                if player_activation.loc[new, player_id.loc[new, player_id.loc[new] == id].index[0]] == False:
...
def update_fa(bat, pit, player_id, player_activation, doc):
    ...
    total_fa = pd.concat([bat_fa.assign(**{'WAR/WAR*': bat_fa['WAR*']})[["Name", "Team", "POS", "WAR/WAR*"]], pit_fa.assign(**{'WAR/WAR*': pit_fa['WAR']})[["Name", "Team", "POS", "WAR/WAR*"]]])
    ...
    for team in live_war.index:
        for column in live_war.columns:
            ...
            if np.isnan(pd.to_numeric(ID)):
                ...
                if ID in bat.index:
                    bat_war = float(bat.loc[ID, "WAR*"])
                else:
                    bat_war = 0
                if ID in pit.index:
                    pit_war = float(pit.loc[ID, "WAR"])
                else:
                    pit_war = 0
...
def update_db(player_name, player_id, current_war, bat, pit):
    ...
    else_fa = pd.concat([OF_fa.iloc[6:].assign(**{'WAR/WAR*': OF_fa['WAR*']})[["Name", "Team", "POS", "WAR/WAR*"]], IF_fa.iloc[8:].assign(**{'WAR/WAR*': IF_fa['WAR*']})[["Name", "Team", "POS", "WAR/WAR*"]], C_fa.iloc[3:].assign(**{'WAR/WAR*': C_fa['WAR*']})[["Name", "Team", "POS", "WAR/WAR*"]], pit_fa.iloc[12:].assign(**{'WAR/WAR*': pit_fa['WAR']})[["Name", "Team", "POS", "WAR/WAR*"]]])
