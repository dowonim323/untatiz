...
async def on_ready():
    ...
    #await channel.send("untatiz bot now online : " + str(datetime.now(timezone(timedelta(hours=9))).strftime('%Y-%m-%d %H:%M:%S'))) 
                ...
                for user_id, team_names in subscribers.items():
                    ...
                    if user:
                        for team_name in team_names:
                            ...
                            # 스프레드시트 데이터를 pandas DataFrame으로 변환
